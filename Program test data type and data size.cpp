...
void DecToBin(int data2, int byt) //Convert Decimal to Binary as byt
Bytes {
  int count, chk, bits;
  bits = (byt * 8) - 1; //Calculate total bits started at bit no 0
  for (count = bits; count >= 0; count--) {
    chk = data2 >> count; //Shift right count bits of data2 and
    keep in to chk
...
void SFloat() //Test Data in Single Precision Float Mode
{
  ...
  printf("Size = %d Bytes\n", sizeof(data)); //Show size
  of unsigned short integer
  getch(); //Wait for press any key14
}
int main() {
    printf("\nDATA REPRESENTATION PROGRAM...\n");
    printf("================================\n");
    while (ch != '0') {
      printf("\n[1]:BinaryToHex [2]:Boolean [3]:Unsigned
        Int[4]: Character\ n ");
